...
class CosmosPolicy(nn.Module):
    ...
    def _query_vlm(self, obs: torch.Tensor, video_frames=None):
        ...
        print(f"VLM output: {message_str}")
        return message_str
    ...
    def forward(self, tensordict: TensorDict) -> TensorDict:
        if self.steps_since_llm >= self.llm_interval:
            ...
            print(f"[Agent] Reasoning repr: {repr(self.current_reasoning)}")  # PRINT FIX 1: in case this is weird non-string format
            self.reasoning_log.append({
                "step":      self.steps_since_llm,
                "reasoning": self.current_reasoning,
            })

            # LLM-AS-A-JUDGE STRETCH: send image frames (?) + observation vector + high-level reasoning
